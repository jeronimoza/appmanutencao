...
uses
  ...
  Vcl.DBGrids, Vcl.StdCtrls, Vcl.ExtCtrls, Vcl.DBCtrls, FireDAC.Comp.Client;
...
type
  ...
  private
    FDataset1, FDataset2: TFDMemTable;
...
begin
  inherited;
  FDataset1 := TFDMemTable.Create(Self);
  FDataset2 := TFDMemTable.Create(Self);
  FDataSource1 := TDataSource.Create(Self);
  FDataSource2 := TDataSource.Create(Self);
...
begin
  ...
  FDataset2.CloneCursor(FDataset1);
